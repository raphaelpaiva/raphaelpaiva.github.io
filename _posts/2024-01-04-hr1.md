---
layout: post
title: "O Gravador sem Cabeça #1 - Dá pra gravar áudio da linha de comando?"
date: 2024-01-04
---
...
* `Subdevices: 1/1`
  - Ele tem um subdispositivo
* `Subdevice #0: subdevice #0`
  - Este subdispositivo é o número 0
...
`hw:CARD=CODEC,DEV=0` tem **muita** cara de ser um identificador de dispositivo. `CODEC` é o nome da placa que vimos na saída do `arecord -l` e 0 é o número do dispositivo, então `hw:CARD=CODEC,DEV=0` deve significar a fonte de áudio direto do hardware que vem da placa CODED, dispositivo 0. Da mesma forma, `plughw:CARD=CODEC,DEV=0` deve significar a mesma fonte de áudio, mas com conversões de software, seja á o que isso signifique.
...
<a href="/files/hr/opa.wav" target="_blank">Ouve aí a alegra do nerdola</a>

Parece até o áudio do Yuri Gagarin com qualidade tosca. Tudo bem, 8 bits de resolução de taxa de amostragem de de 8kHz não dá pra esperar muita coisa.
...
Vamos mexer só no formato por enquando que foi o que ele reclamou.
...
Deu um warning, mas gravou. Vou chorar 😭. <a href="/files/hr/opahw.wav" target="_blank">Ouve aí o choro do nerdola</a>.
...
Vamos brincar um pouco com os outros parametros.
